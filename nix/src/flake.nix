  ...
  outputs = { self, nixpkgs, flake-utils, home-manager, darwin, fenix, ... }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # has GUI
        hasGUI = builtins.pathExists "/tmp/.X11-nix"
          || builtins.getEnv "WAYLAND_DISPLAY" != ""
          || nixpkgs.legacyPackages.${system}.stdenv.isDarwin;
        ...
        # for Home Manager
        mkHomeConfiguration = system: hostname: username: email:
      ...
      in {
        legacyPackages = {
          nixosConfigurations = {
            devvm = mkNixosConfiguration system "devvm" "terassyi";
          };
          homeConfigurations = {
            "terassyi@dev" =
              mkHomeConfiguration system userInfo.hostname userInfo.name
              "dev@terassyi.net";
            "terassyi@devvm" =
              mkHomeConfiguration system "devvm" userInfo.name userInfo.email;
            "terashima@fukdesk" =
              mkHomeConfiguration system "fukdesk" "terashima"
              "terashima-tomoya@cybozu.co.jp";
            "terashima@darwin1" =
              mkHomeConfiguration system "darwin1" "terashima" userInfo.email;
            "terassyi@teradev" =
              mkHomeConfiguration system "teradev" userInfo.name userInfo.email;
          };
        };
      });
}
